...
def make_new_complex_loss_function(mse_weight = 0., corr_weight = 0., manifold_weight = 0., bound=1):
    ...
    def loss_func(y_hat, y_true):
        ...
        total_loss = mse_weight * mse + corr_weight * corr_neg

        return total_loss, corr, mse
    return loss_func
...
def new_train_regression(EPOCHS, model, train_loader, val_loader,
                                 loss_function, train_step, optimizer,
                                 device, raw_test_data, labels, inference_function,
                                 to_many,scheduler = None,
                                 show_info=1, num_losses=10):
...
#     wandb.watch(model, loss_function, log_freq=16)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        # model training
        model.train()
        for counter, (x_batch, y_batch, trunk_batch) in enumerate(train_loader):
            x_batch = x_batch.to(device, dtype=torch.float)
            y_batch = y_batch.to(device, dtype=torch.float)

            # Use previous predictions if available; else fallback to trunk_batch
            if previous_predictions is not None:
                trunk_batch = previous_predictions.to(device, dtype=torch.float).view(batch_size, -1) 
            else:
                trunk_batch = trunk_batch.to(device, dtype=torch.float)  # Default if no prior predictions
                trunk_batch = trunk_batch.view(batch_size, -1) 

            # Perform training step
            #print(x_batch.shape)
            #print(y_batch.shape)
            #print(trunk_batch.shape)
 
            losses = train_step((x_batch, trunk_batch), y_batch, model, optimizer, loss_function)
            num_losses = len(losses)
            ...
            # Save the current predictions for use in the next batch
            """
            with torch.no_grad():
                previous_predictions = model((x_batch, trunk_batch)).detach()
            """
            previous_predictions = y_batch
        ...
        with torch.no_grad():
            for counter_val, (x_batch_val, y_batch_val, trunk_batch_val) in enumerate(val_loader):
                x_batch_val = x_batch_val.to(device, dtype=torch.float)
                y_batch_val = y_batch_val.to(device, dtype=torch.float)

                # Use previous predictions if available; else fallback to trunk_batch
                if previous_predictions is not None:
                    trunk_batch_val = previous_predictions.to(device, dtype=torch.float).view(16, -1) 
                else:
                    trunk_batch_val = trunk_batch_val.to(device, dtype=torch.float)  # Default if no prior predictions
                    trunk_batch_val = trunk_batch_val.view(16, -1) 

                #print(y_batch_val.shape)
                # SCUFFED LMAO
                if y_batch_val.shape[0] != 16:
                    continue

                trunk_batch_val = trunk_batch_val.to(device, dtype=torch.float)
                #print("____")
                #print(trunk_batch_val.shape)
                #print(y_batch_val.shape)
                #print("______")
                y_hat_val = model((x_batch_val, trunk_batch_val))
                losses_val = loss_function(y_hat_val, y_batch_val)
                
                for i in range(len(losses_val)):
                    sum_losses_val[i] = sum_losses_val[i] + losses_val[i].item()

                previous_predictions = y_hat_val
