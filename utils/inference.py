...
def new_make_inference_seq_2_seq(model, dataset, device = 'cpu'):
    """
    Make inference for model many2many. So we take sequency of input data and get sequence of output.
    Then we compare prediction and real output. 
    Model should be fully convolution for working with any input data. 
    ...
    with torch.no_grad():
       ...
       # if model have attribute window size we apply window sliding.
        ...
        print(f"SLIDING WINDOW: {make_window_sliding}")
        ...
        if make_window_sliding:
            ...
            for start in range(0, bound, window_size):
                ...
                trunk_input = torch.FloatTensor(2048 * 8).uniform_(-2, 2).to(device)

                if previous_predictions is not None:
                    trunk_input = previous_predictions
                ...
                trunk_batch = torch.unsqueeze(trunk_input, 0)  

                #print(f"Trunk Shape{trunk_batch.shape}")

                # input should be eeg, trunk_inputs = inputs  # eeg: [batch_size, n_channels], trunk_inputs: [batch_size, time_steps]
                y_hat = model((x_batch, trunk_batch))[0]
                y_hat = y_hat.to('cpu').detach().numpy()
                previous_predictions = torch.tensor(y_hat).to(device).view(-1)
                y_hats.append(y_hat)
    ...
    # Ensure `y_hats` matches the first two dimensions of `y_test`
    y_hats = y_hats[..., :y_test.shape[-1]]

    return y_hats, y_test

def new_model_inference_function(model, dataset, labels, device='cuda', to_many=True):
    """
    Make inference on test data. 
    Takes model input and output. 
    ...
    """
    if to_many:
        y_hats, y_test = new_make_inference_seq_2_seq(model, dataset, device=device)
    else:
        y_hats, y_test = make_inference_seq_2_one(model, dataset, device=device)
